module RVideo # :nodoc:
  ...
  class Inspector
    ...
    def initialize_with_file(file, ffmpeg_binary = nil)
      ...
      # Using egrep to remove metadata subitems info in order to avoid UTF-8 errors
      @raw_response = `#{@ffmpeg_binary} -i #{@full_filename.shell_quoted} 2>&1 | egrep -v "SubItem|\s+:.*"`
    end
